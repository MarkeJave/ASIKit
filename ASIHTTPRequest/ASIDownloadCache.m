    ...
    if (!sharedCache) {
		...
		@synchronized(self) {
			...
			if (!sharedCache) {
				...
				[sharedCache setStoragePath:[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"DefaultWebCache"]];
			}
		}
	}
	return sharedCache;
...
- (NSString *)pathToCachedResponseDataForURLString:(NSString *)url;{

    return [self pathToCachedResponseDataForURL:[NSURL URLWithString:url]];
}
...
+ (NSString *)pathToCachedResponseHeadersForURL:(NSString *)url;{

    return [self keyForURL:[NSURL URLWithString:url]];
}
