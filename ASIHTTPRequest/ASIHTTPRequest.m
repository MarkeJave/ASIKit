...
- (void)addRequestHeader:(NSString *)header value:(NSString *)value{
    ...
    if (!requestHeaders) {
        ...
        [self setRequestHeaders:[NSMutableDictionary dictionary]];
    }
    ...
    if([[[[self url] scheme] lowercaseString] isEqualToString:@"https"]) {
        ...
        if (![self validatesSecureCertificate]) {
            ...
            NSDictionary *sslProperties = [[NSDictionary alloc] initWithObjectsAndKeys:
                                           ...
                                           (NSString *)kCFStreamSSLLevel, (NSString *)[self sslSecurityLevel],
                                           nil];
...
- (void)updateDownloadProgress{
    ...
    if (bytesReceivedBlock) {
        ...
        [self performBlockOnMainThread:^{ if (bytesReceivedBlock) { bytesReceivedBlock(self,value, totalSize); }}];
    }
#endif
...
- (void)updateUploadProgress{
    ...
    if(bytesSentBlock){
        ...
        [self performBlockOnMainThread:^{ if (bytesSentBlock) { bytesSentBlock(self,value, totalSize); }}];
    }
#endif
}
...
- (void)removeUploadProgressSoFar{
    ...
    if(bytesSentBlock){
        ...
        unsigned long long totalSize = [self postLength];
        [self performBlockOnMainThread:^{  if (bytesSentBlock) { bytesSentBlock(self,progressToRemove, totalSize); }}];
    }
#endif
}
...
- (void)scheduleReadStream{
    ...
    if ([self readStream] && ![self readStreamIsScheduled]) {
        ...
        [connectionsLock lock];
        runningRequestCount++;
        if (shouldUpdateNetworkActivityIndicator && [self networkActivityIndicatorVisible]) {
...
- (BOOL)extractIdentity:(SecIdentityRef *)outIdentity andTrust:(SecTrustRef*)outTrust fromPKCS12Data:(NSData *)inPKCS12Data{
    ...
    if (securityError == 0) {
        ...
        *outTrust = (SecTrustRef)tempTrust;
        ...
        ASI_DEBUG_LOG(@"Failed with error code %d",(int)securityError);
        return NO;
    }
    return YES;
}
...
+ (NSString *)defaultUserAgentString{
    ...
    @synchronized (self) {
        ...
        if (!defaultUserAgent) {
            ...
            //            NSString *appIdentifier = [bundle objectForInfoDictionaryKey:@"CFBundleIdentifier"];
            ...
            [self setDefaultUserAgentString:[NSString stringWithFormat:@"appName:%@;version:%@;buildVersion:%@;deviceName:%@;OS:%@ %@;local:%@", appName, marketingVersionNumber, developmentVersionNumber, deviceName, OSName, OSVersion, locale]];
        }
...
+ (void)recordBandwidthUsage{
    ...
    averageBandwidthUsedPerSecond = totalBytes/MAX(0.0001, measurements);
}
...
+ (void)registerForNetworkReachabilityNotifications{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:kASIReachabilityChangedNotification object:nil];
}

+ (void)unsubscribeFromNetworkReachabilityNotifications{
    
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kASIReachabilityChangedNotification object:nil];
}
